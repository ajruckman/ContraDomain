...
DROP TABLE IF EXISTS contralog.log_actions_per_sliding_hour;
CREATE VIEW IF NOT EXISTS contralog.log_actions_per_sliding_hour AS
SELECT toStartOfHour(cast(time AS DateTime)) + (now() - toStartOfHour(now())) AS h,
       action,
       count(*)                                                               AS c
FROM contralog.log
